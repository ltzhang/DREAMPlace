# GangSTA timer backend for DREAMPlace (timer_engine=gangsta). Mirrors timing_heterosta but drives
# GangSTA's open C API (gangsta_set_netlist_inmem / gangsta_extract_rc_from_placement). The C++ op
# applies the GangSTA<->DREAMPlace pin permutation (seam C.4) and 0-based cell indexing (seam C.1).
import torch
from torch.autograd import Function
from torch import nn
import numpy as np
import dreamplace.ops.timing_gangsta.timing_gangsta_cpp as timing_gs_cpp
import logging
import pdb
...
def _build_timing_io_args(params):
    """
    @brief Build argv list for GangSTA timing IO
    @param params the parameters defined in json
    @return argv list suitable for timing_gs_cpp.io_forward
    """
    args = ["DREAMPLACE"]
...
class TimingIO(Function):
    """
    @brief The timer IO class for GangSTA integration
    GangSTA reads some external files like liberty libraries, SDC, etc. 
    ...
    def read(params, placedb):
        ...
        return timing_gs_cpp.io_forward(
            args,
            placedb.rawdb,
            dreamplace_mappings
        )
...
class TimingOptFunction(Function):
    ...
    def forward(ctx, timer, pos, 
        ...
        """
        @brief compute timing analysis using GangSTA
        @param timer the GangSTA timer object
        ...
        if use_cuda and not pos_is_cuda:
            logging.info("GangSTA: Converting pos to CUDA for GPU timing analysis")
            pos = pos.cuda()
        elif not use_cuda and pos_is_cuda:
            logging.info("GangSTA: Converting pos to CPU for CPU timing analysis")
            pos = pos.cpu()
...
class TimingOpt(nn.Module):
    def __init__(self, timer, net_names, pin_names, flat_netpin,
        ...
        """
        @brief Initialize the feedback module for GangSTA timing analysis
        @param timer the GangSTA timer object
    ...
    def forward(self, pos):
        """
        @brief call GangSTA timing forward function
    ...
    def update_net_weights(self, max_net_weight=np.inf, n=1):
        """
        @brief update net weights of placedb using GangSTA simplified algorithm
        ...
        try:
            ...
            timing_gs_cpp.update_net_weights(
            ...
            logging.error(f"GangSTA net weight update failed: {e}")
    ...
    def report_wns_tns(self):
        ...
        """
        return timing_gs_cpp.report_wns_tns(
    ...
    def report_wns(self):
        ...
        """
        return timing_gs_cpp.report_wns(
            self.timer.raw_timer, bool(self.use_cuda))
    ...
    def report_tns(self):
        ...
        """
        return timing_gs_cpp.report_tns(
            self.timer.raw_timer, bool(self.use_cuda))
    ...
    def time_unit(self):
        ...
        """
        return 1e-12  # GangSTA uses picosecond as unit



    # NOTE: GangSTA's C API does not yet implement gangsta_dump_paths_to_file (only gangsta_report_paths
    # is implemented), so dump_paths_setup_to_file is intentionally omitted. The timing-driven flow does
    # not require it.
