...
using namespace _timing_gangsta_impl;

// Wrapper class to hold the unique_ptr for Python binding
class TimerWrapper {
public:
    TimingGangstaIO::GangstaTimerPtr timer_;
    
    TimerWrapper(TimingGangstaIO::GangstaTimerPtr timer) : timer_(std::move(timer)) {}
    
    GangstaTimer* get_raw_timer() const { return timer_.get(); }
    GangstaTimer& get_raw_timer_ref() const { return *timer_; }
};

///
/// @brief Parse arguments and create GangSTA timer with PlaceDB integration and DREAMPlace mappings
...
std::shared_ptr<TimerWrapper> _timing_io_forward_heterosta(const pybind11::list& args, PlaceDB& placedb, 
    ...
    // Initialize GangSTA
    auto timer_ptr = TimingGangstaIO::initialize_heterosta();
    if (!timer_ptr) {
        dreamplacePrint(kERROR, "CRITICAL ERROR: GangSTA initialization failed!\n");
        // Clean up arguments
        for (int i = 0; i < argc; ++i) {
            delete[] argv[i];
        }
    ...
    if (!TimingGangstaIO::setupTiming(*timer_ptr, placedb, argc, argv, dreamplace_mappings)) {
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    ...
    pybind11::class_<DREAMPLACE_NAMESPACE::TimerWrapper, std::shared_ptr<DREAMPLACE_NAMESPACE::TimerWrapper>>(m, "Timer")
        ...
        .def("update_arrivals", [](DREAMPLACE_NAMESPACE::TimerWrapper& wrapper, bool use_cuda) {
            gangsta_update_arrivals(wrapper.get_raw_timer(), use_cuda);
        ...
        .def("get_is_endpoint", [](DREAMPLACE_NAMESPACE::TimerWrapper& wrapper, pybind11::array_t<bool> is_endpoint) {
            ...
            gangsta_get_is_endpoint(wrapper.get_raw_timer(), static_cast<bool*>(buf.ptr));
        ...
        .def("count_endpoints", [](DREAMPLACE_NAMESPACE::TimerWrapper& wrapper, size_t num_pins) -> size_t {
            ...
            gangsta_get_is_endpoint(wrapper.get_raw_timer(), reinterpret_cast<bool*>(is_endpoint.data()));
        ...
        }, "Get raw GangstaTimer pointer as integer for C++ functions")
        
        
        // SPEF file operations
        .def("write_spef", [](DREAMPLACE_NAMESPACE::TimerWrapper& wrapper, const std::string& spef_path) {
            return gangsta_write_spef(wrapper.get_raw_timer(), spef_path.c_str());
        ...
        .def("read_spef", [](DREAMPLACE_NAMESPACE::TimerWrapper& wrapper, const std::string& spef_path) {
            return gangsta_read_spef(wrapper.get_raw_timer(), spef_path.c_str());
    ...
    m.def("forward", [](DREAMPLACE_NAMESPACE::TimerWrapper& wrapper, torch::Tensor pos,
        ...
        DREAMPLACE_NAMESPACE::TimingGangstaCpp::forward(
    ...
    }, "GangSTA timing forward analysis",
    ...
    m.def("update_net_weights", [](DREAMPLACE_NAMESPACE::TimerWrapper& wrapper, int n,
        ...
        DREAMPLACE_NAMESPACE::TimingGangstaCpp::update_net_weights(
    ...
    }, "Update net weights using GangSTA",
    ...
    // GangSTA IO function with PlaceDB integration and DREAMPlace mappings
    ...
    m.def("report_wns_tns", [](DREAMPLACE_NAMESPACE::TimerWrapper& wrapper, bool use_cuda) {
            ...
            if (success) {
                ...
                DREAMPLACE_NAMESPACE::dreamplacePrint(DREAMPLACE_NAMESPACE::kERROR, "Failed to query WNS/TNS from GangSTA\n");
